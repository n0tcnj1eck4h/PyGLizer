...
class CPPGenerator(GeneratorBase):
    ...
    def pointer_declaration(cls, command: Command):
        return f'extern {cls.pointer_typedef_name(command)} {cls.internal_pointer_name(command)};\n'

    @classmethod
    def pointer_definition(cls, command: Command):
        return f'{cls.pointer_typedef_name(command)} {cls.internal_pointer_name(command)} = nullptr;\n'
